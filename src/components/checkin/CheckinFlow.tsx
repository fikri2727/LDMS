...
import { verifyCheckin, submitPublicSurvey, type CheckinState } from "@/app/checkin/actions";
import { SurveyForm } from "@/components/training/SurveyForm";

const initialState: CheckinState = {};

export function CheckinFlow({ trainingId, trainingTitle }: { trainingId: number; trainingTitle: string }) {
  const boundVerify = verifyCheckin.bind(null, trainingId);
  ...
  async function handleSurveySubmit(formData: FormData) {
    setSubmitError(null);
    try {
      await submitPublicSurvey(trainingId, state.staffNo!, formData);
      setDone(true);
    } catch (e) {
    ...
    return (
      <div className="text-center py-6">
        ...
        <p className="text-sm text-text-secondary">
          You&apos;ve already submitted your evaluation for this training. Thank you.
        </p>
      </div>
    );
  }
  ...
  if (state.verified && state.staffNo) {
    return (
      <div>
        <h2 className="text-lg font-semibold text-text-primary mb-1">{trainingTitle}</h2>
        ...
        <SurveyForm action={handleSurveySubmit} />
      </div>
    );
  }
  ...
  return (
    <form action={formAction} className="space-y-4">
      <div>
        <h2 className="text-lg font-semibold text-text-primary mb-1">{trainingTitle}</h2>
        <p className="text-sm text-text-secondary mb-4">Enter your Staff ID to fill your post-training evaluation.</p>
